(function($){
    ...
    TreeView.prototype= {
        ...
        operation:function(obj,index){
            ...
            if(len==2){
                this.$dom.find(".itemBox").css("background","#e6f3ff")
            }else if(len>=3){
                this.$dom.find(".itemBox").css("background","rgb(238,243,249)")
            }
            //之后
            this.$dom.find(".icon-icon").css({
                left:16+8*(len-1)+"px",
            })
            this.$dom.find(".title").css({
                "margin-left":30+8*(len-1)+"px",
               // width:"70%"
        ...
        create:function(options){
            ...
            this.$dom=$('<div class="zjltest itemBox lev'+options.lev+'" lev="'+options.lev+'">'+
