...
<body>
	<div class="tableBox">
		<div class="operationBox layui-form">
			...
			<shiro:hasPermission name="affairs:add">
		...
		<table class="layui-table center" id="table">
			...
			<c:choose>
				<c:when test="${not empty affairs_List}">
					<shiro:hasPermission name="affairs:select">
						...
						<c:forEach items="${affairs_List}" var="var" varStatus="vs">
							...
							<tr>
								...
								<td style="width: 160px; text-align: center;">
										...
										<shiro:hasPermission name="affairs:updata">
											<a class="layui-btn layui-btn-mini edit"  data_id="${var.id }">编辑</a>
										</shiro:hasPermission>

										<shiro:hasPermission name="affairs:del">
					...
					<shiro:lacksPermission name="affairs:select">
						<tr>
		...
		layui.use('laypage', function() {
			...
			$("#add").on("click", function() {
                parent.layer.open({
                    title:"新增优待信息",
                    type: 2,
                    content:'<%=basePath%>affairs/toAdd.do?currentPage=${page.currentPage}'+sum,
                    area: ["1000px","600px"]
                });
                transmit(window);
			})
            ...
            $("#table").on("click", ".selUser", function() {
                ...
                parent.layer.open({
                    title:"优待信息",
                    type: 2,
                    content:'<%=basePath%>affairs/toSelect.do?id='+id,
                    area: ["800px","500px"]
                });
                transmit(window);
            })
			...
			$("#table").on("click", ".edit", function() {
                ...
                parent.layer.open({
                    title:"编辑优待信息",
                    type: 2,
                    content:'<%=basePath%>affairs/toEdit.do?id='+id+'&currentPage=${page.currentPage}'+sum,
                    area: ["1000px","600px"]
                });
                transmit(window);
			})
                ...
                parent.layer.confirm("是否确认删除此数据", {
                    ...
                    $.get(url, function (data) {
                        if ("successs" == data.result) {
                            window.location.href = "<%=basePath%>affairs/list.do?currentPage=${page.currentPage}"+sum;
