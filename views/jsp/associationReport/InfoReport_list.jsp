...
<div class="tableBox">
    ...
    <hr>
    <div style="text-align:center;"><span style="color: #FF3333;">${sysArea.name}</span>老年协会信息统计表</div>
...
<script type="text/javascript">
    ...
    layui.use('laydate', function () {
        ...
        $("#dowExcel").on("click", function () {
            ...
            if (version != null && "" != version) {
                window.location.href = '<%=basePath%>associationReport/dowExcel.do?version=' + version + '&level=${pd.level}';
            } else {
    ...
    $("#tbody").on("click", ".address", function () {
        var id = $(this).data("id");
        var title = $(this).text();
        parent.addTab(title + "老年协会信息统计表", "associationReport/listInfo.do?area_code=" + id + "&year=" + '${pd.year}' + "&nd=" + '${pd.nd}');
    });
