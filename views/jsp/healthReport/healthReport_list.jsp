...
<div class="tableBox">
    ...
    <hr>
    <div style="text-align:center;"><span style="color: #FF3333;">${sysArea.name}</span>健康档案信息统计表</div>
            ...
            <c:when test="${not empty list}">
                <c:forEach items="${list}" var="var" varStatus="vs">
                    <tr>
                        ...
                        <td>
                            <c:if test="${var.yjl_num == '0'}">
                                0.00%
                            </c:if>
                            <c:if test="${var.yjl_num != '0'}">
                                <fmt:formatNumber value="${var.yjl_num/var.total_num*100}" pattern="0.00" maxFractionDigits="2"/>%
                            </c:if>
                        </td>
                    </tr>
                </c:forEach>
...
<script type="text/javascript">
    ...
    layui.use('laydate', function () {
        ...
        $("#dowExcel").on("click", function () {
            ...
            if (version != null && "" != version) {
                window.location.href = '<%=basePath%>healthReport/dowExcel.do?version=' + version + '&level=${pd.level}';
            } else {
    ...
    $("#tbody").on("click", ".address", function () {
        var id = $(this).data("id");
        var title = $(this).text();
        parent.addTab(title + "健康档案信息统计表", "healthReport/listInfo.do?area_code=" + id + "&year=" + '${pd.year}' + "&nd=" + '${pd.nd}');
    });
