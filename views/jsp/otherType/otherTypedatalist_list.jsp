...
<script type="text/javascript">
    ...
    layui.use('laypage', function () {
        ...
        laypage.render({
            ...
            jump: function (obj, first) {
                if (!first) {
                    window.location.href = "<%=basePath%>otherType/list.do?currentPage=" + obj.curr + '&SECTION_ID=' + '${SECTION_ID}' + '&keywords=' + '${keywords}';
                }
            }
        });
    });
    ...
    function refreshIfream(SECTION_ID) {
        window.location.href = "<%=basePath%>otherType/list.do?SECTION_ID=" + SECTION_ID;
    }
    ...
    $("#add").on("click", function () {
        ...
        parent.parent.layer.open({
            title: "新增",
            type: 2,
            content: '<%=basePath%>otherType/toAdd.do?SECTION_ID=' + '${SECTION_ID}' + '&currentPage=' + ${page.currentPage},
            area: ["800px", "550px"]
        });
    });
    var first= true;
    ...
    function edit(SECTION_ID, TYPE) {
        ...
        parent.parent.layer.open({
            title: msg,
            type: 2,
            content: '<%=basePath%>otherType/toEdit.do?SECTION_ID=' + SECTION_ID + '&currentPage=' + ${page.currentPage} +'&TYPE=' + TYPE,
            area: ["800px", "550px"]
        });
    }
    ...
    function del(SECTION_ID) {
        parent.parent.layer.confirm("是否删除此外部单位?", {
            ...
            var url = "<%=basePath%>otherType/delete.do?SECTION_ID=" + SECTION_ID;
            $.get(url, function (data) {
                if("hasChildren" == data.result){
                    parent.parent.layer.msg('本单位下有人员，请先删除人员信息!', {icon: 2,zIndex:"200000000"});
                }else if ("success" == data.result) {
                    window.location.href = "<%=basePath%>otherType/list.do?SECTION_ID=${SECTION_ID}&currentPage=${page.currentPage}";
