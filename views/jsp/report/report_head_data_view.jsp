...
<style>
	.report_edit_head .h2{padding:9px 15px;font-size:13px;color:#333}
...
<div class="tableBox report_edit_head" >
			...
			<div class="layui-colla-content layui-show" style="overflow:hidden">
					<form id="form">
						<div class="layui-row marT-10">
							...
							<c:if test="${agewhere==true }">
								<div class="layui-col-xs6 layui-col-sm6 layui-col-md6 padB-20">
									<label class="layui-form-label" style="width:80px">年龄：</label>
									<div class="layui-input-block" style="margin-left:120px">
										<div class="layui-col-xs5 ">
											<input  name="startage"    type="number"   lay-verify="required" placeholder="起始年龄" autocomplete="off" class="layui-input">
										</div>
										<div class="layui-col-xs1 " style="padding:10px">
											<hr class="hr" />
										</div>
										<div class="layui-col-xs5 ">
											<input  name="endage"  type="number"   lay-verify="required" placeholder="结束年龄" autocomplete="off" class="layui-input">
										</div>
									</div>
								</div>
							</c:if>
	...
	<table class="layui-table center ms-controller" id="table" ms-controller="table"  style="margin-bottom:30px">
		<thead>
		<tr>
			<th ms-for="(index,item) in @list" :attr="{colspan:(item.children==0?1:item.children),rowspan:(item.children==0?2:1)}" :class="(item.active? 'active':'')" >
				{{item.name}}
			</th>
...
<script type="text/javascript">
    avalon.ready(function(){
		...
		$("#sure").on("click",function(){
			...
			layer.load(2)
			$.get("<%=basePath%>customReportStatis/reportList?headId="+'${id}'+"&"+data,function(data){
