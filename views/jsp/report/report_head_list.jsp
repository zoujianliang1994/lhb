...
<script type="text/javascript">
    ...
    layui.use('laypage', function () {
        ...
        laypage.render({
            ...
            jump: function (obj, first) {
                if (!first) {
                    window.location.href = "<%=basePath%>customReport/headList.do?currentPage=" + obj.curr + '&keywords=' + '${keywords}' + '&xzqh_code=' + '${pd.xzqh_code}';
                }
            }
        });
    });
    ...
    function edit(id,name){
        ...
        parent.showSuspensionBox("customReport/headView?id="+id,title)
        parent.layer.transmit= window;
    }

    function analysis(id,title){
        parent.addTab(title+"图形分析","customReport/getHeadDataView?id="+id)
    }
    ...
    function del(id) {
        parent.parent.layer.confirm("是否删除此表头?", {
            ...
            var url = "<%=basePath%>customReport/deleteRH.do?id=" + id;
            $.get(url, function (data) {
                if (data) {
                    window.location.href = '<%=basePath%>customReport/headList.do?currentPage=${page.currentPage}' ;
