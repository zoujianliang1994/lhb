...
<body>
	<div class="tableBox">
		<div class="operationBox layui-form">
			...
			<form action="info.do" method="post" name="Form" id="Form">
			...
			<shiro:hasPermission name="volunteer:info:add">
		...
		<table class="layui-table center" id="table">
			...
			<c:choose>
				<c:when test="${not empty volunteer_List}">
					<shiro:hasPermission name="volunteer:info:select">
						...
						<c:forEach items="${volunteer_List}" var="var" varStatus="vs">
							...
							<tr>
								<td>${vs.index+1}</td>
								<td>${var.areaName }</td>
								<td>${var.activityTimes}</td>
								<td>${var.volunteerCount }</td>
								<td>${var.servesCount }</td>
								<td>${var.applyDate }</td>
								<td>${var.applyYear }</td>
								...
								<td style="width: 160px; text-align: center;">
										...
										<shiro:hasPermission name="volunteer:info:updata">
											<a class="layui-btn layui-btn-mini edit"  data_id="${var.id }">编辑</a>
										</shiro:hasPermission>

										<shiro:hasPermission name="volunteer:info:del">
					...
					<shiro:lacksPermission name="volunteer:info:select">
						<tr>
		...
		layui.use('laypage', function() {
            ...
            laypage.render({
                ...
                jump: function (obj, first) {
                    if (!first) {
                        window.location.href = "<%=basePath%>volunteer/info.do?currentPage=" + obj.curr + "&type=${pd.type}&selAreaCode=${pd.selAreaCode}&selAreaName=${pd.selAreaName}";
                    }
                }
            });
			...
			$("#add").on("click", function() {
                parent.layer.open({
                    title:"上报信息",
                    type: 2,
                    content:'<%=basePath%>volunteer/toAddInfo.do?currentPage=${page.currentPage}&type=${pd.type}&selAreaCode=${pd.selAreaCode}&selAreaName=${pd.selAreaName}',
                    area: ["1000px","320px"]
                });
                transmit(window);
			})
            ...
            $("#table").on("click", ".selUser", function() {
                ...
                parent.layer.open({
                    title:"查看",
                    type: 2,
                    content:'<%=basePath%>volunteer/toSelectInfo.do?id='+id,
                    area: ["1000px","320px"]
                });
                transmit(window);
            })
			...
			$("#table").on("click", ".edit", function() {
                ...
                parent.layer.open({
                    title:"编辑信息",
                    type: 2,
                    content:'<%=basePath%>volunteer/toEditInfo.do?id='+id+'&currentPage=${page.currentPage}&type=${pd.type}&selAreaCode=${pd.selAreaCode}&selAreaName=${pd.selAreaName}',
                    area: ["1000px","320px"]
                });
                transmit(window);
			})
			...
			$("#table").on("click", ".del", function() {
                ...
                parent.layer.confirm("是否确认删除此数据", {
                    ...
                    var url = "<%=basePath%>volunteer/deleteInfo.do?id=" + id + "&guid=" + new Date().getTime();
                    $.get(url, function (data) {
                        if ("success" == data.results) {
                            window.location.href = "<%=basePath%>volunteer/info.do?currentPage=${page.currentPage}&type=${pd.type}&selAreaCode=${pd.selAreaCode}&selAreaName=${pd.selAreaName}";
